...
internal extension String {
    ...
    func findAll(_ regex: String, flags: RegularExpression.Options) -> [String] {
        let re = try! RegularExpression(pattern: regex, options: flags)
    ...
    func split(_ regex: String) -> [String] {
        let re = try! RegularExpression(pattern: regex, options: .dotMatchesLineSeparators)
        let all = NSMakeRange(0, self.characters.count)
        var result = [String]()
        let matches = re.matches(in: self, options: [], range: all)
        if matches.count > 0 {
